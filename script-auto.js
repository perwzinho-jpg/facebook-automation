...
async function automateAutoRetry(email, password, proxyUrl = null, browserscanUrl = null) {
  ...
  try {
    ...
    if (businessId) {
      ...
      const siteName = `${projectName}.vercel.app`;

      logger.info(`   📝 Nome do site: ${siteName}\n`);
      ...
      try {
        ...
        logger.warn('   ⚠️ Template não encontrado, criando HTML padrão...\n');
...
<body style="font-family: Arial; max-width: 600px; margin: 50px auto; text-align: center;">
  ...
  <p><strong>Porte:</strong> {{PORTE}}</p>
      ...
      // Substituir placeholders com informações da empresa
      htmlContent = htmlContent
        .replace(/{{NOME_FANTASIA}}/g, cnpjData.nomeFantasia || cnpjData.razaoSocial)
        .replace(/{{RAZAO_SOCIAL}}/g, cnpjData.razaoSocial)
        .replace(/{{CNPJ}}/g, cnpjData.cnpj)
        .replace(/{{EMAIL}}/g, cnpjData.email || 'contato@empresa.com')
        .replace(/{{TELEFONE}}/g, cnpjData.telefone || '(XX) XXXX-XXXX')
        .replace(/{{SITUACAO}}/g, cnpjData.situacao || 'Ativa')
        .replace(/{{DATA_ABERTURA}}/g, cnpjData.dataAbertura)
        .replace(/{{PORTE}}/g, cnpjData.porte || 'MEI')
        .replace(/{{DATA_GERACAO}}/g, new Date().toLocaleDateString('pt-BR'));

      // Salvar HTML em um diretório temporário
      const projectDir = path.join(process.cwd(), `vercel-${projectName}`);
      ...
      logger.info('   ✅ HTML gerado\n');
        ...
        const preencheuUrl = await page3.evaluate((dominio) => {
          // Procurar input
          let input = null;
        ...
        if (preencheuUrl.success) {
          // Digitar usando keyboard com delays MAIORES
          logger.info('   ⌨️ Digitando URL caractere por caractere...');
          for (let i = 0; i < dominioLimpo.length; i++) {
            const char = dominioLimpo[i];
            await page3.keyboard.type(char);
            await new Promise(r => setTimeout(r, 150 + Math.random() * 200)); // Muito maior
          }
          logger.info('   ✅ URL digitada com sucesso\n');

          // Aguardar um pouco para garantir que tudo foi registrado
          await new Promise(r => setTimeout(r, 500));
          ...
          const clicouAdicionar = await page3.evaluate(() => {
            ...
            // Procurar pelo botão "Adicionar" que está visível e ativo
            for (let i = allButtons.length - 1; i >= 0; i--) {
              const btn = allButtons[i];
              const text = btn.textContent?.trim().toLowerCase() || '';

              if (text === 'adicionar' && btn.offsetParent !== null) {
          ...
          if (clicouAdicionar) {
            ...
            logger.warn('   ⚠️ Botão "Adicionar" não encontrado ou desabilitado\n');
          }

          // Aguardar mais tempo para o Facebook gerar a meta tag
          logger.info('   ⏳ Aguardando 10 segundos para Facebook gerar meta tag...\n');
